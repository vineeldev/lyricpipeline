...
3. Before deploying, add these environment variables under Settings, then Environment Variables:
   ...
   - ANTHROPIC_API_KEY: optional, powers the Deal Description column. Create a key at console.anthropic.com (add a small amount of credit; usage here costs pennies per month). If this variable is not set, the description column simply shows dashes and everything else works normally.
...
Deals grouped by the ten pipeline stages, with deal name, a one-line description of the artist or songwriter, next steps, and status. Stages with dollar values also show Equity, RBN, and TEV columns with section totals. Deals marked Passed/Dead or completed are excluded automatically. Stage and strategy filters (Lyric, MMM, SOS) are available at the top.

## Deal descriptions

The Deal Description column is generated automatically from the deal names using the Claude API. Descriptions are cached for a week and regenerate automatically whenever the set of deal names in Asana changes, so new deals get a description without anyone doing anything. Names the model does not confidently recognize show a dash rather than a guess. Since these lines are AI generated and viewer facing, skim them once after deploying.
