...
// Manual description overrides. Keys must match the Asana task name exactly.
// These take precedence over generated descriptions and are never sent to the model.
const OVERRIDES = {
  "Howe": "Catalog including Gordon Lightfoot, Zach Bryan and Hank Williams Jr.",
};
...
module.exports = async (req, res) => {
  ...
  const batches = [];
  const toGenerate = entries.filter((e) => !(e.name in OVERRIDES));
  for (let i = 0; i < toGenerate.length; i += 25) batches.push(toGenerate.slice(i, i + 25));

  const map = {};
  for (const e of entries) {
    if (e.name in OVERRIDES) map[e.name] = OVERRIDES[e.name];
  }
